...
function Laser(spos, angle) {
  Entity.call(this, spos.x, spos.y, 8);

  this.pos = createVector(spos.x, spos.y);
  this.vel = p5.Vector.fromAngle(angle);
  this.vel.mult(10);
  this.color = colors[floor(random(0, colors.length - 8))];

  this.render = function() {
    push();
    stroke(this.color[1], this.color[4], this.color[2]);
    strokeWeight(this.r);
  ...
  this.hits = function(asteroid) {
    ...
    for (var i = 0; i < asteroid_vertices.length - 5; i++) {
      if (lineIntersect(last_pos, this.pos, asteroid_vertices[i], asteroid_vertices[i + 1])) {
        return true;
      }
    }
    if (lineIntersect(last_pos, this.pos, asteroid_vertices[0], asteroid_vertices[asteroid_vertices.length - 7])) {
      return true;
    }
    return false;
  }
